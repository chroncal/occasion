...
class DataAugmentForObjectDetection():
    def __init__(self, rotation_rate=0.5, max_rotation_angle=5,
                 ...
                 is_addNoise=False, is_changeLight=True, is_cutout=True, is_rotate_img_bbox=True,
                 is_crop_img_bboxes=True, is_shift_pic_bboxes=True, is_filp_pic_bboxes=False):
    ...
    # 旋转
    def _rotate_img_bbox(self, img, bboxes, angle=10, scale=1.):
        '''
    ...
    def dataAugment(self, img, bboxes):
        ...
        while change_num < 1:  # 默认至少有一种数据增强生效
            ...
            if self.is_changeLight:
                if random.random() > 0:  # 改变亮度
                    change_num += 1


                    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
                    saturation_scale = np.random.uniform(0.65, 1.3)

                    h, s, v = cv2.split(hsv)
                    v = (v * saturation_scale).astype(np.uint8)
                    v = np.clip(v, 0, 255)
                    s = (s * saturation_scale).astype(np.uint8)
                    s = np.clip(s, 0, 255)
                    modified_hsv = cv2.merge((h, s, v))
                    img = cv2.cvtColor(modified_hsv, cv2.COLOR_HSV2BGR)
                    # 修改对比度
                    lab = cv2.cvtColor(img, cv2.COLOR_BGR2Lab)
                    l, a, b = cv2.split(lab)

                    contrast_factor = np.random.uniform(0.65, 1.3)
                    ...
                    img = cv2.cvtColor(modified_lab, cv2.COLOR_Lab2BGR)
            if self.is_addNoise:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_img_path', type=str, default=r'D:\nanodet-0.2.0\nanodet-0.2.0\dataset\img')
    parser.add_argument('--source_xml_path', type=str, default=r'D:\nanodet-0.2.0\nanodet-0.2.0\dataset\label')
    parser.add_argument('--save_img_path', type=str, default=r'D:\nanodet-0.2.0\nanodet-0.2.0\dataset\img')
    parser.add_argument('--save_xml_path', type=str, default=r'D:\nanodet-0.2.0\nanodet-0.2.0\dataset\label')
